...
import { getLogTextColor, getLogLevelClass, copyToClipboard } from "../../utils"
...
const TestConnectionFailureModal = ({
	...
	const fromSources = connectionType === "source"
	...
	const handleCopyLogs = async () => {
		const logs = JSON.stringify(testConnectionError?.logs || [], null, 4)
			...
			>
				...
				<div className="flex w-full flex-col items-center">
					...
					<h2 className="text-center text-xl font-medium">
						{connectionType === "source" ? "Source" : "Destination"} test
						...
						>
							{!isExpanded ? (
								...
								<table className="min-w-full">
									<tbody>
										{(testConnectionError?.logs || []).map((jobLog, index) => (
											<tr key={index}>
