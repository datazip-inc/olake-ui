...
// This cross-cutting concern requires access to both ETL and optimisation handlers.
func (h *Handler) CreateDestinationAndCatalog() {
	...
	if h.Optimisation != nil && configJSON != "" {
		...
		if _, err := h.Optimisation.GetService().CreateCatalogFromOLakeConfig(h.Ctx.Request.Context(), configJSON, true); err != nil {
